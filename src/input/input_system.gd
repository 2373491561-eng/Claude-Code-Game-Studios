## InputSystem — polls Godot Input Map actions in _physics_process() and exposes
## game input as clean typed getters.
##
## This node is owned by game.tscn (not an Autoload), per ADR-0002 and ADR-0006.
## All game action detection happens here — systems read cached results rather
## than calling Input.is_action_*() directly. This guarantees consistent values
## within a single physics tick and enforces the ADR-0002 rule that
## _input(event) only sets lightweight state markers.
##
## State machine (Story 003):
##   NORMAL         — all inputs available
##   DODGING        — only aim + skill_1 (move/shoot locked, no new dodge)
##   SKILL_CASTING  — aim + dodge (cancel) + shoot + move (skill_1 blocked)
##   PAUSED         — only pause action (to resume)
##   DEAD           — no inputs
##
## Pause handling (Story 004):
##   Pause toggle is in _physics_process() before all other input processing.
##   Focus loss auto-pauses; focus return stays paused (manual Esc to resume).
##   InputSystem uses PROCESS_MODE_ALWAYS so _physics_process() runs even
##   when get_tree().paused = true (so it can detect Esc to unpause).
##
## Usage:
##   [codeblock]
##   # In game.tscn _ready():
...
##
##   # In PlayerMovement._physics_process():
##   velocity = input_sys.get_move_axis() * move_speed
##
##   # In DodgeSystem._physics_process():
##   if input_sys.is_dodge_just_pressed():
##       execute_dodge()
##
##   # External systems set state:
##   input_sys.set_state(InputSystem.STATE_DODGING)
##
##   # ShootingSystem checks for flash-resume:
##   if input_sys.consume_shoot_resume():
##       start_shooting()
##   [/codeblock]
extends Node
